...
# Criar diret�rio para ffmpeg se necess�rio e definir permiss�es
RUN mkdir -p /MediaToolkit && chmod -R 755 /MediaToolkit

# Mover o ffmpeg para o diret�rio correto, se necess�rio
RUN ln -s $(which ffmpeg) /MediaToolkit/ffmpeg.exe

# Criar diret�rio Downloads e ajustar permiss�es
RUN mkdir -p /app/Downloads && chown -R app:app /app/Downloads

# Definir permissões recursivas para Downloads
